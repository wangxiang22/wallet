 ...
 */
@Component
public class LuaScriptRedisService {
    ...
    /**
     * 对于hash key 执行计数，并增加过期时间
     * @param key
     * @return
     */
    public Long doHashIncr(String key,String hashKey,Long second,Long incrNum){
        StringBuilder luaScript = new StringBuilder();
        luaScript.append(" local incrNum = redis.call('hincrby',KEYS[1],KEYS[2],ARGV[2]);");
        luaScript.append(" local timeResidueValue = redis.call('ttl',KEYS[1]);");
        luaScript.append(" if timeResidueValue == -1 then ");
        luaScript.append(" redis.call('expire',KEYS[1],ARGV[1])");
        luaScript.append(" end;");
